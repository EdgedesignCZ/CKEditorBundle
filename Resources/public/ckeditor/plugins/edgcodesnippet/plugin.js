...
( function() {
    ...
    CKEDITOR.plugins.add( 'edgcodesnippet', {
        ...
        icons: 'edgcodesnippet', // %REMOVE_LINE_CORE%
        hidpi: true, // %REMOVE_LINE_CORE%

        beforeInit: function( editor ) {
            editor._.edgcodesnippet = {};

            /**
             * Sets the custom syntax highlighter. See {@link CKEDITOR.plugins.edgcodesnippet.highlighter}
             ...
             * the three callbacks).
             * * This method is accessible through the `editor.plugins.edgcodesnippet` namespace only.
             *
             * @since 4.4
             * @member CKEDITOR.plugins.edgcodesnippet
             * @param {CKEDITOR.plugins.edgcodesnippet.highlighter} highlighter
             */
            this.setHighlighter = function( highlighter ) {
                editor._.edgcodesnippet.highlighter = highlighter;

                var langs = editor._.edgcodesnippet.langs =
                ...
                editor._.edgcodesnippet.langsRegex = new RegExp( '(?:^|\\s)lang-(' +
        ...
        init: function( editor ) {
            editor.ui.addButton && editor.ui.addButton( 'CodeSnippet', {
                label: editor.lang.edgcodesnippet.button,
                command: 'codeSnippet',
                toolbar: 'insert,10'
            } );
        },
     ...
     *
     * For more information see the [Code Snippet Guide](#!/guide/dev_edgcodesnippet).
     *
     * @class
     * @singleton
     */
    CKEDITOR.plugins.edgcodesnippet = {
        highlighter: Highlighter
    };

    /**
     * A Code Snippet highlighter. It can be set as a default highlighter
     * using {@link CKEDITOR.plugins.edgcodesnippet#setHighlighter}, for example:
     *
     ...
     *              var myHighlighter = new CKEDITOR.plugins.edgcodesnippet.highlighter( {
     *                  init: function( ready ) {
     ...
     *              editor.plugins.edgcodesnippet.setHighlighter( myHighlighter );
     *          }
     *      } );
     *
     * @since 4.4
     * @class CKEDITOR.plugins.edgcodesnippet.highlighter
     * @extends CKEDITOR.plugins.edgcodesnippet
    ...
    function Highlighter( def ) {
         ...
         * in the [Code Snippet documentation](#!/guide/dev_edgcodesnippet-section-changing-languages-list).
         *
    ...
    function registerWidget( editor ) {
        var codeClass = editor.config.codeSnippet_codeClass,
            ...
            lang = editor.lang.edgcodesnippet;

        editor.widgets.add( 'codeSnippet', {
            allowedContent: 'pre(prettyprint,lang-*); code',
            ...
            template: '<pre class="prettyprint ' + codeClass + '"><code></code></pre>',
            dialog: 'codeSnippet',
            ...
            highlight: function() {
                ...
                editor._.edgcodesnippet.highlighter.highlight( widgetData.code, widgetData.lang, function( formatted ) {
            ...
            data: function() {
                ...
                if ( newData.pre )
                    this.parts.pre.setHtml( CKEDITOR.tools.htmlEncode( newData.pre ) );

                // Remove old .lang-* class.
                if ( oldData && newData.lang != oldData.lang )
                    this.parts.pre.removeClass( 'lang-' + oldData.lang );

                // Lang needs to be specified in order to apply formatting.
                if ( newData.lang ) {
                    // Apply new .lang-* class.
                    this.parts.pre.addClass( 'lang-' + newData.lang );
            ...
            // Upcasts <pre><code [class="lang-*"]>...</code></pre>
            upcast: function( el, data ) {
                ...
                // Read lang-* from <code> class attribute.
                var matchResult = editor._.edgcodesnippet.langsRegex.exec( el.attributes[ 'class' ] );

                console.log(editor._.edgcodesnippet);
                ...
                console.log('add codeClass', codeClass);
                el.addClass( codeClass );

                return el;
            },

            // Downcasts to <pre><code [class="lang-*"]>...</code></pre>
            downcast: function( el ) {
                ...
                console.log('remove codeClass', codeClass);
 ...
 * See {@link CKEDITOR.plugins.edgcodesnippet.highlighter} to read more.
 *
 * @since 4.4
 * @cfg {String} [codeSnippet_codeClass='hljs']
 * @member CKEDITOR.config
 */
CKEDITOR.config.codeSnippet_codeClass = 'prettyprint';
 ...
 * in the [Code Snippet documentation](#!/guide/dev_edgcodesnippet-section-changing-highlighter-theme).
 *
 * @since 4.4
